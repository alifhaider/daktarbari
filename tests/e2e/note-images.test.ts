...
test('Users can create note with an image', async ({ page, login }) => {
	...
	await page
		.getByLabel('image')
		.nth(0)
		.setInputFiles('tests/fixtures/images/alif-notes/cute-koala.png')
...
test('Users can create note with multiple images', async ({ page, login }) => {
	...
	await page
		.getByLabel('image')
		.nth(1)
		.setInputFiles('tests/fixtures/images/alif-notes/koala-coder.png')
...
test('Users can edit note image', async ({ page, login }) => {
	...
	const updatedImage = {
		altText: 'koala coder',
		location: 'tests/fixtures/images/alif-notes/koala-coder.png',
	}
...
function createNoteWithImage() {
	return {
		...
		images: {
			create: {
				altText: 'cute koala',
				objectKey: 'alif-notes/cute-koala.png',
			},
		},
	} satisfies Omit<
		Note,
